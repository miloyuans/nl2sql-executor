...
async function exportSchema(){try{eResult.textContent='正在导出，请稍候...';let data=await api('/v1/admin/schema/export',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({data_source_id:eDatasource.value,include_system_schemas:eSystem.checked})});eResult.textContent=JSON.stringify(data,null,2);loadExports()}catch(e){eResult.textContent=e.message;toast(e.message)}}
async function loadExports(){try{let data=await api('/v1/admin/schema/exports');exportsBody.innerHTML=(data.exports||[]).map(function(f){return '<tr><td class="mono">'+esc(f.file)+'</td><td>'+fmtSize(f.size)+'</td><td>'+fmtTime(f.updated_at)+'</td><td><a class="download" href="'+esc(f.download_url)+'" target="_blank">下载</a></td></tr>'}).join('')||'<tr><td colspan="4" class="hint">暂无导出文件</td></tr>'}catch(e){}}
